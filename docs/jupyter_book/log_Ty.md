...
**Mình đã học được gì:** Cách sử dụng Kafka Connect và cấu hình Kafka Sink để đẩy dữ liệu tự động vào đồ thị Neo4j mà không cần code thêm.
**Khó khăn và giải pháp:** Khó khăn khi đảm bảo dữ liệu không bị trùng lặp khi chạy lại nhiều lần (idempotency). Giải pháp là dùng lệnh Cypher MERGE thay vì CREATE trong Sink properties, và định nghĩa Schema Json nghiêm ngặt.
